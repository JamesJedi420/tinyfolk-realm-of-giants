...
**Current implementation status:** TIN-161 adds bounded Tinyfolk defensive prop obstacles through `DefensivePropService` and deterministic shared logic in `Shared/GiantRealm/DefensivePropState`. Authored prop anchors (`DefensiveProp_*`) pair with zones (`DefensivePropZone_*`) for drop-triggered obstacles with Ready, Dropped, Blocked, Broken, Resetting, and Disabled lifecycle states. Tinyfolk drop requests validate role, range, recharge, and per-player active caps; Giants clear dropped props through timed start/complete break actions. Barrier startup remains traversable for linked Giant macro routes; active windows block zone-scoped Giant macro navigation through `_DefensivePropService_QueryAPI` while Tinyfolk escape routes remain unaffected. Active Blocked-window Giant contact applies bounded prop-radius stagger and can interrupt in-progress break actions via `ResolveGiantBarrierContact`. Qualifying rescue-context drops grant rescue acceleration through `_RescueContractService_QueryAPI.GrantRescueAcceleration`. `InteractionResolver` routes F-key defensive prop drop/break through `DefensivePropRequest` remotes (server-created by `DefensivePropService`). Prop-type catalog includes per-family timing overrides (shelf, rope_gate) and authored anchors A/B/C. Arbitrary placement, persistence, and full presentation polish remain deferred.
...
**Current implementation status:** TIN-61 adds bounded authored containment structures through `ContainmentStructureService` and deterministic shared logic in `Shared/GiantRealm/ContainmentStructureState`. Structure anchors (`ContainmentStructure_*`) pair with zones (`ContainmentStructureZone_*`) for capacity-tracked occupants, escape-interaction eligibility, and teammate rescue checks. `CaptureService.PromoteTargetToContained` assigns occupants when structures are registered and clears occupants on custody end. TIN-87 slice 1 adds `ContainmentZoneInteractionService` for grab-place containment delivery, contained structure escape progress (`ContainmentZoneEscapeState`), structure rescue requests, and minimal client remotes (`ContainmentZoneInteractionClient`). Build-mode placement slice adds `ContainmentStructurePlacementState`, `ContainmentStructureConfig.BuildModePlacements`, and `GiantBuildModeService` materialization hooks that register placed cage/jar catalog entries with `ContainmentStructureService.RefreshRegistries`. Additional structure families, persistence, and presentation polish remain deferred.
...
**TIN-106 realm transfer handoff tokens status:** `RealmTransferHandoffState` and `RealmTransferHandoffService` now own short-lived handoff tokens, durable realm session records keyed by `realmId`, and transfer-lock acquire/consume on arrival. `RealmTransferSessionStore` backs session records with `MemoryStoreStructurePolicy.realmTransferSessions` (in-memory fallback when adapter unavailable). Lifecycle updates invalidate session tokens on abort/logout/failed-enqueue paths; destination confirm resolves expired sessions via `pendingRecovery` authority before safe-fallback routing. `ProfileTeleportHandoffService` wires controlled-transfer begin/confirm to `_RealmTransferHandoffService_QueryAPI` and `_RealmTransferAssignmentService_QueryAPI.ApplyTransition` (including `ConfirmHubReturn` for escape-return) while keeping TeleportData non-authoritative per TIN-49.
...
**Current implementation status:** TIN-40 expands the canonical inventory craft catalog to six recipes and twelve items, including craft-chain recipes that consume prior crafted outputs (`rope_anchor`, `field_kit`, `spike_wedge`), crafted-item consumption flow (`ConsumeItems`), craftable-recipe listing (`ListCraftableRecipes`), catalog cross-validation helpers, and debug snapshot/attribute exposure for craftable recipe counts. Public query API craft paths remain canonical-catalog guarded. Final UX polish, persistence, and specialist-visual equip coupling remain deferred.

**Current implementation status:** TIN-27 adds session `Metal` flow in `ResourceFlowState` (produced → in-transit → stored), `Forge_A` refining (stored Stone → Metal produced pool via Miner `refine` labor), warehouse haul/delivery for Metal, warehouse delivery grants to Tinyfolk inventory through `_InventoryCraftService_QueryAPI.GrantItems` (`iron_filings` from Metal; `stone_shard` from Stone), and Metal debug attributes on station/warehouse surfaces. TIN-256 projects session Metal stored into Giant treasury HUD via `GiantRealmTreasuryPresentation`, `GiantBuildModeService` attribute projection, and `GiantHudStackClient` Treasury section (including corrected session Wood/Stone reads from flow snapshot). Crafting HUD polish and ledger Metal remain deferred.

**Current implementation status:** Inventory craft material counts now persist on the Tinyfolk player profile through `ProfilePersistenceGateway` / `InventoryCraftProfileState`. `InventoryCraftService` hydrates session inventory from profile on ownership-ready and role-change-to-Tinyfolk, bootstraps first-time players via `TEST_STARTING_ITEMS`, and writes item counts back on grant/consume/craft and player leave. Crafted-item metadata totals, crafting HUD polish, and specialist-visual equip coupling remain session-only or deferred.

**Current implementation status:** ConstructionService performs startup discovery from `_G._DistrictPlacementService_QueryAPI.GetAllPlacedDistricts` and initializes minimal district build-state entries. DistrictPlacementService remains placement authority and query-only provider. ConstructionService owns build-state initialization only in this slice. ConstructionConfig is cost authority for district defaults via `DistrictDefaultCost`; `DistrictBuildState` entries carry resolved `woodCost`, `stoneCost`, `foodCost`, and `metalCost` at startup; cost is not derived from placement state. ConstructionService routes `ConstructionBuildRequest` through both `siteStateById` (existing site-backed path) and `districtBuildStateById` (district path); district build mutation is server-owned and spends startup cost fields; range check for districts is deferred (no Workspace part reference in this slice); DistrictPlacementService is not modified. `handleDebugRequest` now covers both site and district IDs in both the targeted (string payload) and snapshot-all (nil payload) paths; district entries use a `districtId` field to distinguish them from site entries; unknown IDs in both paths continue to produce `site_not_found`.

**Current implementation status:** TIN-39 adds four-material (wood, stone, food, metal) construction spend through deterministic shared logic in `Shared/Construction/ConstructionSpendState`, site/district cost resolution in `ConstructionConfig`, and optional metal on `ResourceFlowState.SpendStoredAtomic` / `AddStoredAtomic` with rollback symmetry. `ConstructionService` uses shared affordability checks, spends all four stored materials on site and district builds, exposes metal cost and stored debug attributes, and registers `ConstructionSite_C` as a metal-requiring site. Multi-stage builds, per-district cost override tables, persistence, UX polish, and `GiantBuildModeService` ledger unification remain deferred.
...
**Current implementation status:** TIN-28 adds a bounded realm-scoped social economy layer through deterministic shared logic in `Shared/GiantRealm/SocialEconomyState`, config in `Shared/Config/SocialEconomyConfig`, runtime orchestration in `SocialEconomyService`, and persistence via `socialEconomyPersistence` on `GiantRealmSaveSchema` (TIN-249). Favor/control axes (0–100) drive effect snapshots (`favorComplianceBonus`, `controlIntimidationBonus`) with axis decay, bonus caps, and trophy delta diminishing returns. Trophy producers map capture/release outcomes into social events. Gameplay consumers include capture intimidation (PenRationing morale gate), labor production yield, ransom post/settle gates, TIN-254 dyadic-favor worker ranking, and TIN-255 control-dominant worker deprioritization in `TinyfolkJobSchedulingState` when multiple Tinyfolk compete for the same scheduled job demand. Parallel to but separate from `GiantReputationState`, `PenRationingState` custodian morale, `TinyfolkStatusService` fear exposure, and player-profile reputation tracks. TIN-251 projects per-Tinyfolk dyadic favor/control from `dyadicByUserId` onto Tinyfolk player attributes (`SocialDyadicFavor`, `SocialDyadicControl`) for Giant inspect display; covenant/miracle/domain expansion paths and full presentation polish remain deferred.

**Current implementation status:** TIN-92 adds a bounded Giant village status HUD projection through deterministic shared logic in `Shared/GiantRealm/VillageStatusState` and `VillageStatusHudPresentation`, config in `Shared/Config/VillageStatusHudConfig`, server orchestration in `VillageStatusProjectionService`, and read-only client consumption in `VillageStatusHudClient`. The service aggregates existing resource flow, pen rationing/morale, active Tinyfolk raider count, and job-scheduling debug snapshots onto realm-owner Giant player attributes with danger bands for food, morale, shortfall, raiders, and coarse escape pressure. TIN-252 consolidates the right-stack detail panels (Treasury, Pen Rationing, Social Economy, Giant Effects) into `GiantHudStackClient` with collapsible sections reusing existing presentation modules; standalone right-edge HUD clients are retired. TIN-257 persists accordion expand/collapse per section on the player profile via `GiantHudStackPreferenceProfileState`, `GiantHudStackPreferenceService`, and client remote/attribute projection. Rebellion pressure, unified theming/animation, and Tinyfolk-facing village summaries remain deferred.

**Current implementation status:** TIN-102 adds a bounded Giant-only Tinyfolk inspect panel through deterministic shared logic in `Shared/GiantRealm/TinyfolkInspectState` and `TinyfolkInspectPresentation`, config in `Shared/Config/TinyfolkInspectConfig`, and read-only client selection/consumption in `TinyfolkInspectClient`. Giants click-select in-range Tinyfolk characters to view replicated per-player attributes (role, traits, assignment, custody, score/tokens, sprint fatigue, captive stress proxies). TIN-250 adds client-only selection highlight (`Highlight` on selected character, tone-synced with inspect panel accent) in `TinyfolkInspectClient`. TIN-251 wires loyalty display from server-projected `SocialDyadicFavor` / `SocialDyadicControl` attributes via `SocialEconomyService` dyadic projection. TIN-253 adds `ResolveHighlightTone` so world-space highlight color reflects dyadic dominant axis (favor → positive, control → warning) while custody stress tones still win for panel accent and for highlight when non-neutral. Per-Tinyfolk morale simulation, Tinyfolk teammate inspect, and server-curated inspect snapshots remain deferred.
